# B45 Labs – Privacy Policy

**Effective date:** 2026-01-01
**Last updated:** 2026-05-01

This Privacy Policy explains how B45 Labs ("B45 Labs", "we", "our", "us") collects and uses information when you use the B45 Labs Revit add-in (the "Software").

## 1. Scope
This Policy applies to information collected through the Software, including usage analytics and error logs used solely for internal product development, stability improvements, and user support.

## 2. What We Collect

### 2.1 Usage Data
- Commands executed (name/identifier) and usage counts.
- Timestamped usage events (e.g., when a command was executed).
- Add-in version and build number.
- Revit version (e.g., "Autodesk Revit 2026").

### 2.2 Error and Diagnostic Data
- Error logs, exception messages, and stack traces generated by the Software.
- The command that was active when the error occurred.
- Add-in version, build number, and Revit version at the time of the error.

### 2.3 Installation and Update Data
- First-seen and last-updated timestamps for each installation.
- Current add-in version and, when applicable, the version from which an upgrade occurred.

### 2.4 Pseudonymous Identifiers
Each installation is assigned a randomly generated installation identifier (`install_id`) and each session a session identifier (`session_id`). These are pseudonymous — they do not contain your name, email, or any personally identifiable information. They are used solely to group usage and error records for troubleshooting and analytics.

### 2.5 User Profile Data (Voluntary)
If you choose to fill out the optional User Profile within the Software, the following information is collected:
- Full name.
- Preferred name (used for the avatar and in-app greeting).
- Email address.
- Company / studio name.
- Role / title.
- Experience with Revit (years of use).
- How you found B45 Labs (one or more selected discovery tags; may include a
  free-text detail for "Other" and "Conference / talk").

Providing this information is entirely voluntary. If you do not fill out the User Profile, no personal information is collected. Your profile is saved locally in `%AppData%\B45Labs\user-profile.json` and can be edited or cleared at any time from the User Profile window.

### 2.6 Region Data
- Country, continent, and region (e.g., state, province, or metropolitan area) to understand geographic usage distribution. This information is derived from your IP address at the time of data submission; your IP address is not stored.

## 3. What We Do NOT Collect
We do not intentionally collect any sensitive personal data, such as health data, biometric identifiers, precise location data, government IDs, financial account data, or authentication credentials.

We do not intentionally collect:
- Payment information.
- Personal documents or model contents.
- Project file contents, drawings, geometry, or BIM data.
- Revit model names, element counts, link counts, or any project/model metadata.
- File paths, folder locations, or file names.
- Precise geolocation (street address / GPS coordinates).
- Passwords, authentication credentials, or personal communications.
- Device or system characteristics (CPU, RAM, OS version, screen resolution).
- Revit usernames.

If such information appears in a user-provided support screenshot or log, please redact it before sharing.

Donations/support (if offered) are processed by third-party providers. B45 Labs does not receive or store payment details.

## 4. How We Use Information
We use collected information strictly for internal purposes, including:
- Measuring feature adoption and command usage to prioritize development.
- Improving stability, performance, and reliability across Revit versions
  (2023–2026, plus experimental support for 2027).
- Debugging and fixing errors and regressions.
- Validating installer/update distribution health.
- Producing aggregated, anonymized metrics for roadmap planning.

## 5. Sharing and Disclosure
We do not sell your data and we do not share collected data with third parties for advertising or marketing.

We may use trusted service providers to store or process analytics. Such providers are used only to support our internal operations and are not authorized to use the information for their own purposes.

We may disclose information if required by law, court order, or to protect our rights and the security of the Software.

## 6. Data Retention
We retain data only as long as necessary for internal analytics, troubleshooting, and product improvement.

As a general guideline:
- Usage and diagnostics data may be retained for up to **24 months** unless a longer retention is required to investigate recurring issues or to comply with legal obligations.

## 7. Security
Collected data is stored on access-controlled infrastructure and is limited to authorized personnel for the purposes described in this Policy. We implement reasonable administrative, technical, and organizational safeguards to protect collected information. However, no method of transmission or storage is 100% secure.

## 8. Your Choices and Requests
You may contact us to request information about data associated with your usage, or to request correction or deletion where applicable. We may ask for additional information to verify your request.

## 9. International Use
The Software may be used in different countries. Information may be processed in the United States or other locations where our service providers operate.

## 10. Changes to This Policy
We may update this Policy from time to time. The "Last updated" date will reflect the most recent revision. Material changes may be communicated through release notes or in-Software notices.

## 11. Contact
**B45 Labs Support**
Email: support@B45Labs.com
